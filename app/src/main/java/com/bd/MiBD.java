...
public class MiBD extends SQLiteOpenHelper {
    ...
    public void insercionMovimiento(Movimiento m){
        db.execSQL("INSERT INTO movimientos (rowid, id, tipo, fechaoperacion, " +
                "descripcion, importe, idcuentaorigen, idcuentadestino) " +
                "VALUES (null, null, " +
                m.getTipo() + ", " +
                m.getFechaOperacion().getTime() + ", '" +
                m.getDescripcion() + "', " +
                m.getImporte() + ", " +
                m.getCuentaOrigen().getId() + ", " +
                m.getCuentaDestino().getId() + ");");
    }

    public void actualizarSaldo(Cuenta c){
        db.execSQL("UPDATE cuentas SET saldoactual= " + c.getSaldoActual() +
                " WHERE banco='" + c.getBanco() +
                "' AND sucursal='" + c.getSucursal() +
                "' AND dc='" + c.getDc() +
                "' AND numerocuenta='" + c.getNumeroCuenta() + "';");
    }

    public boolean existeCuenta(String banco,String sucur,String dc,String numCuenta){
        String sql="SELECT numerocuenta " +
                "FROM cuentas " +
                "WHERE banco='" + banco +
                "' AND sucursal='" + sucur +
                "' AND dc='" + dc +
    ...
    private void insercionDatos(SQLiteDatabase db){
        ...
        db.execSQL("INSERT INTO clientes(id, nif, nombre, apellidos, claveSeguridad, email) VALUES (4, '55555555E', 'Admin', 'Admin', '1234', 'admin.admin@tia.es');");
